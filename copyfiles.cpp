...
: QMainWindow(parent), m_nStep(0)
{
	ui.setupUi(this);	
	// create the GUI
	QSettings settings("CopyFiles", "Settings");
	settings.beginGroup("StartPath");
	restoreGeometry(settings.value("geometry").toByteArray());
	bool bCheckHistory = settings.value("check").toBool();
	...
	m_autoHide->setChecked(bCheckHistory);
}
...
CopyFilesWindow::~CopyFilesWindow()
{
	...
	settings.setValue("check", m_autoHide->getChecked());
}
...
void CopyFilesWindow::on_pushButton_Help_clicked()
{
	static QString help = QString::fromLatin1("<h4>") + tr(LocalString_CN("特定正则使用规则\n")) + QString::fromLatin1("</h4>"
		"<p>") + tr(LocalString_CN("+:")) + tr(LocalString_CN("<b>（test/*.+）或（test/*.后缀+）</b> 自动创建根目录(test)")) + QString::fromLatin1("</p>"	
		"<p>") + tr(LocalString_CN("-:")) + tr(LocalString_CN("<b>（test/*.-）或（test/*.后缀-）</b>  只拷贝根目录(test)下的匹配项")) + QString::fromLatin1("</p>"
		"<p>") + tr(LocalString_CN(">:")) + tr(LocalString_CN("<b>（test/>新目录(多级)）或（test/文件>新目录(多级)）或  <br> （test/*.后缀>新目录(多级)）</b> 拷贝到(多级)新目录")) + QString::fromLatin1("</p>"
		"<p>");
	QMessageBox msgBox;
	msgBox.setText(help);	
